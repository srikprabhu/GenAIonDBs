...
query_generation_prompt = PromptTemplate(
    input_variables=["query", "all_tab_col","table_column_descriptions","chat_history"],
...
NL_response_template="""I will give you a query & it's answer which is extracted through a sql query along with the table's info from which it's extracted from & the descriptions of each column of the table. You will have to answer the query in English.
...
NL_response_prompt=PromptTemplate(
    input_variables=["query","result","sql_query","table_column_details","chat_history"],
    template=NL_response_template
)

NL_response_chain=LLMChain(prompt=NL_response_prompt,llm=llm)

def final_response(query,chat_history):

    sql_query_table=query_generation_chain.invoke({"query":query,"all_tab_col":all_tab_col,"table_column_descriptions":table_column_descriptions,"chat_history":chat_history})['text']
...
#     print(result)#debugging
    ...
    final_response = NL_response_chain.invoke({"query":query,"result":result,"sql_query":sql_query,"table_column_details":table_column_details,"chat_history":chat_history})['text']
...
if __name__ == "__main__":
    ...
    response=final_response(query,chat_history)
    print(response)